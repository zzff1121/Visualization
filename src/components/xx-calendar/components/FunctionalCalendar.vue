          ...
          <div class="vfc-calendar" v-for="(calendar, key) in listCalendars" :key="key">
            <div class="vfc-content">
              ...
              <section class="vfc-dayNames">
                <span v-for="(dayName, key) in fConfigs.dayNames" :key="key">{{ dayName }}</span>
              </section>
              <transition-group tag="div" class="c-title-anchor" :name="getTransition_()" appear>
...
export default {
  ...
  methods: {
    ...
    setCalendarData() {
      ...
      for (let i = 0; i < this.fConfigs.calendarsCount; i++) {
        ...
        let calendar = {
          ...
          weeks: helpCalendar.getFinalizedWeeks(
            date.getMonth(),
            date.getFullYear()
          )
        }
    ...
    setConfigs() {
      ...
      // Limits
      if (this.fConfigs.limits) {
        this.calendar.currentDate = helpCalendar.getDateFromFormat(
          this.fConfigs.limits.min
        )
      } else {
        // New Date
    ...
    clickDay(item) {
      ...
      if (this.fConfigs.isDateRange) {
        ...
        if (this.calendar.dateRange.start) {
          startDate = helpCalendar
            .getDateFromFormat(this.calendar.dateRange.start)
            .getTime()
        }

        // Two dates is not empty
        if (
          this.calendar.dateRange.start !== false &&
        ...
        if (
          ...
          let firstDate = helpCalendar.getDateFromFormat(
            this.calendar.dateRange.start
          )
          let secondDate = helpCalendar.getDateFromFormat(
          ...
          week.days.forEach(day => {
            ...
            if (vm.fConfigs.isDatePicker) {
              ...
              if (startDate && endDate) {
                if (
                  helpCalendar.getDateFromFormat(day.date).getTime() >
                    helpCalendar.getDateFromFormat(startDate) &&
                  helpCalendar.getDateFromFormat(day.date) <
                    helpCalendar.getDateFromFormat(endDate)
                ) {
                  day.isMarked = true
                }
              }
        ...
        for (let e = 0; e < this.listCalendars.length; e++) {
          ...
          for (let f = 0; f < calendar.weeks.length; f++) {
            ...
            for (let i = 0; i < week.days.length; i++) {
              ...
              if (this.calendar.dateRange.start) {
                let itemDate = helpCalendar
                  .getDateFromFormat(item.date)
                  .getTime()
                let thisDate = helpCalendar.getDateFromFormat(date).getTime()
                let startDate = helpCalendar
                  .getDateFromFormat(this.calendar.dateRange.start)
                  .getTime()

                let dateDay =
                  helpCalendar.getDateFromFormat(item.date).getDay() - 1
                if (dateDay === -1) {
                  dateDay = 6
                }
    ...
    PreYear() {
      ...
      this.calendar.currentDate = new Date(
        this.calendar.currentDate.getFullYear() - 1,
    ...
    getClassNames(day) {
      ...
      if (
        ...
        if (
          ...
          if (
            helpCalendar.getDateFromFormat(
              this.fConfigs.markedDateRange.start
            ) <= helpCalendar.getDateFromFormat(day.date) &&
            helpCalendar.getDateFromFormat(this.fConfigs.markedDateRange.end) >=
              helpCalendar.getDateFromFormat(day.date)
          ) {
            classes.push('vfc-marked')
          }
